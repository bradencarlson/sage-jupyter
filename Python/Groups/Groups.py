...
# We can also create subgroups of groups with SageMath.  We can do this using the `subgroup()` command.  This command must be used on an existing group, and a set of generators must be specified.  For example, if we Have the group $S_5$, as defined above, and we wish to find the subgroup of $S_5$ generated by $(1,2,3)$ and $(2,3)$.  Then we would write:

# In[ ]:


subgroup = S5.subgroup(['(1,2,3)','(2,3)'])
print(subgroup.list())
